...
def parse_args():
    ...
    parser.add_argument('--peft_method', default=None,
                        help='method of peft')
    parser.add_argument('--ft_method', default=None,
                        help='method of ft')
    ...
    parser.add_argument('--mix_model', action='store_true', help='Print verbose output.')
...
def Opt_setting(ft_method,lr):
    lr1 = lr
    groups = []
    for name, param in model.named_parameters():
        if ft_method in name:
...
def test_pipeline(
    ...
    for test_task in task_list:
        ...
        # mertic = evaluator.get(evaluator_name)
        # if mertic is None:
        #     print('mertic is None')
        print("number of evalLoader",len(test_loader))
        print('====val-best model tesing=====')
        logging.info(f"val-best model tesing\n")
        # try:
        # Eval_model(test_loader,processor,model,device,evaluator,evaluator_name)
...
target_modules_map = {
        ...
        'd_qkvo':['out_proj','q_proj', 'k_proj', 'v_proj'],
        'd_qv':['q_proj', 'v_proj'],
        # 'e_qv_d_qkvo':['query', 'value','out_proj', 'query', 'key', 'value'],
        # 'e_qkv_d_qkvo':['query','key' ,'value','out_proj', 'query', 'key', 'value'],
...
if __name__ =='__main__':
    ...
    # get model #
    if mix_model :
        image_processor,tokenizer,model = get_model_and_processor(
    ...
    if not only_test:
        ...
        if ft_method is None:
            ...
            optimizer = Opt_setting(ft_method,lr)
        if mix_model:
        ...
        if use_trainer:
            ...
            accuracy_metric = evaluate.load('./evaluate_metrics/accuracy.py')
            # precision_metric = evaluate.load('./evaluate_metrics/precision.py')
            # recall_metric = evaluate.load('./evaluate_metrics/recall.py')
            def compute_metrics(pred):
                ...
                acc = accuracy_metric.compute(pred_ids.reshape(1,-1).squeeze(),\
                                                label_ids.reshape(1,-1).squeeze(),normalize=True)['accuracy']
                return {
                    "cer": cer,
                    "acc": acc,
                }
            trainer = huggingface_trainer(training_args,model,processor,train_dataset,val_dataset,compute_metrics)
            trainer.train()
        else :
    ...
    logger = setup_logging_to_file(\
        os.path.join(log_file_dir,\
                    'Test'+'_epoch_'+str(epochs)+'_time_'+str(time.time())+'.log'))
    print('====start tesing ====')
    ...
    data_dir:
    IAM:dataset/IAM/
    SROIE:dataset/SROIE_Task2/
    Latex:dataset/Latex/
    STR:dataset/STR_BENCHMARKS/
    Multi:dataset/
    
    '''
    
    if peft_dir is  None:
        peft_dir = save_val_best_dir
    
    # model_dir = model_dir if peft_mode else save_val_best_dir
    print(f'save dir {peft_dir}')
    ...
    if task =='IAM' or task =='Latex' or task=='SROIE' or task =='STR':
        test_pipeline(task,data_dir,processor,model,device,max_len)
    else :
        for task_i in ['IAM','SROIE','STR']:
            data_dir_i = os.path.join(data_dir,taskMapDir[task_i])
            test_pipeline(task_i,data_dir_i,processor,model,device,max_len)
    print('====end eval====')
    ########################################    test  part  ###########################################
    logging.info('End')
    # 删除所有处理器

...
def benchmark_all_eval(model, criterion, converter, opt, calculate_infer_time=False):
    ...
    for eval_data in eval_data_list:
        ...
        _, accuracy_by_best_model, norm_ED_by_best_model, cer_by_best_model, precision_by_best_model, recall_by_best_model, f1_by_best_model, _, _, infer_time, length_of_data = validation(
            model, criterion, evaluation_loader, converter, opt)
        list_accuracy.append(f'{accuracy_by_best_model:0.3f}')
        total_forward_time += infer_time
        total_evaluation_data_number += len(eval_data)
        total_correct_number += accuracy_by_best_model * length_of_data
        print('Acc %0.3f\t normalized_ED %0.3f\t CER %0.3f\t Precision %0.3f\t Recall %0.3f\t F1 %0.3f' % (accuracy_by_best_model, norm_ED_by_best_model, cer_by_best_model, precision_by_best_model, recall_by_best_model, f1_by_best_model))
        print('-' * 80)
...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        for pred, gt in zip(preds_str, labels):
            ...
            for pred, gt in zip(preds_str, labels):
                ...
                n_total_chars += len(gt_chars)
                n_pred_chars += len(pred_chars)
                n_true_chars += len(gt_chars)
    ...
    cer = (n_total_chars - n_correct_chars) / n_total_chars
    precision = n_correct_chars / n_pred_chars if n_pred_chars > 0 else 0
    recall = n_correct_chars / n_true_chars if n_true_chars > 0 else 0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0

    return valid_loss_avg.val(), accuracy, norm_ED, cer, precision, recall, f1, preds_str, labels, infer_time, length_of_data
...
def test(opt):
    ...
    evaluation_loader = torch.utils.data.DataLoader(
        ...
        num_workers=int(opt.workers),
        collate_fn=AlignCollate_evaluation, pin_memory=True)
    _, accuracy_by_best_model, _, cer_by_best_model, precision_by_best_model, recall_by_best_model, f1_by_best_model, _, _, _, _ = validation(
        model, criterion, evaluation_loader, converter, opt)

    print(f'Accuracy: {accuracy_by_best_model:0.3f}, CER: {cer_by_best_model:0.3f}, Precision: {precision_by_best_model:0.3f}, Recall: {recall_by_best_model:0.3f}, F1: {f1_by_best_model:0.3f}')
    with open(f'./result/{opt.experiment_name}/log_evaluation.txt', 'a') as log:
        log.write(f'Accuracy: {accuracy_by_best_model:0.3f}, CER: {cer_by_best_model:0.3f}, Precision: {precision_by_best_model:0.3f}, Recall: {recall_by_best_model:0.3f}, F1: {f1_by_best_model:0.3f}\n')
